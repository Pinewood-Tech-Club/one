...
def get_or_create_user(google_user_id, email, name):
    ...
    cursor = conn.execute("SELECT id, name FROM users WHERE google_user_id = ?", (google_user_id,))
...
def get_user_by_id(user_id):
    ...
    conn = get_conn(Config.MAIN_DB_PATH)
    cursor = conn.execute(
        "SELECT id, google_user_id, email, name, created_at, last_login FROM users WHERE id = ?",
        (user_id,),
    )
    result = cursor.fetchone()
